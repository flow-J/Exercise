...
void wordCounting()
{
    ...
    for (const auto &w : word_count)
        std::cout << w.first << " occurs " << w.second << (w.second > 1 ?
                 "times" : "time");
}

void wordTransformation()
{
    std::ifstream ifs_map("../data/word_transformation.txt"), ifs_content("../data/given_to__transform.txt");
    ...
    for (string key, value; ifs_map >> key && getline(ifs_map, value); )
        if (value.size() > 1) trans_map[key] = value.substr(1),substr(0, value.find_last_not(' '));

    for (string text, word; getline(ifs_content, txt); std::cout << std::endl)
